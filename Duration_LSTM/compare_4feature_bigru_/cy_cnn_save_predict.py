...
def get_arguments():
    ...
    parser.add_argument('--Train', type=_str_to_bool, default=Train,
...
if __name__ == '__main__':
    ...
    ##########################################
    #from eye matrix to number(3d),ex:[0,0,0] 0 ,[0,0,1] 2
    def oneHotToNumber3D(returnResult):
    ...
    predictLabel=oneHotToNumber3D(returnResult)
    targetLabel = oneHotToNumber3D(Y_sample)
    directoryName = "Predict_s1s2position"
    if not os.path.exists(directoryName):
        os.makedirs(directoryName)
    # save for peaple observe
    f = open(directoryName + '/' + "predict.txt", "w")
    f.write(str(predictLabel))
    f = open(directoryName + '/' + "target.txt", "w")
    f.write(str(targetLabel))
    f.close()
    # save for read
    ...
    #         print(i,j)
    #         print(predict[i,j],target[i,j])

    # plt.plot(predict.T)
    # plt.plot(target.T)
    # plt.show()
    # plt.plot(oneResult.T)
    # for i in returnResult[0]:
    #     plt.plot(i)
    # plt.show()
    # print("returnResult",returnResult,oneResult.shape)
    # for i in range(returnResult.shape[0]):
    #     print(np.argmax(returnResult[0][i],axis=0),Y_test[0][i])
