...
if __name__=='__main__':
    path='../kddcup_buffer_overflow_vs_back_5_fold/'
    ...
    f=open(path+'log_smote/log_smote.txt','a')
    for fold in range(5):
        ...
        while True:
            ...
            for i in range(len(x1)):
                tmp_dis=(((x1[idx]-x1[i])**2).sum())**0.5
                dis.append(tmp_dis)
            dis[dis.index(min(dis))]=max(dis)
            for k in range(5):
                min_idx=dis.index(min(dis))
                r=np.random.uniform(0,1)
                new_data=x1[idx]+r*(x1[min_idx]-x1[idx])
                x1_ros.append(new_data)
                y1_ros.append(1)
                if (len(x1_ros)+len(x1))==len(x0):
                    stop=True
                    break
                dis[min_idx]=max(dis)
            if stop:
        ...
        x=np.array(x1_ros).reshape(-1,attr_count)
        y=np.array(y1_ros).reshape(-1,1)
